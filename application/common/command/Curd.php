...
class Curd extends Command
{
    ...
    {
        $this->setName('curd')->setDescription('一键生成增删改查操作');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        foreach ($tables as $k => $v){
            if ($v['table_name'] != 'yh_product'){
                continue;
            }
            $sql = "select COLUMN_NAME name, DATA_TYPE type, CHARACTER_MAXIMUM_LENGTH lenght, COLUMN_COMMENT comment, COLUMN_KEY index_key from INFORMATION_SCHEMA.COLUMNS where table_schema = '".$database."' AND table_name = '".$v['table_name']."'";
            $tmp = Db::query($sql);
            $table = str_replace($prefix, '', $v['table_name']);
